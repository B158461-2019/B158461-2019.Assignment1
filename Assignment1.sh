...
mkdir result_1
gzip -d ./2*.fq.gz
fastqc -f fastq -o result_1 ./2*.fq
...
bowtie2 --fast -x Tbbgenome -1 ./216_L8_1.fq -2 ./216_L8_2.fq -S alignment1.sam
bowtie2 --fast -x Tbbgenome -1 ./218_L8_1.fq -2 ./218_L8_2.fq -S alignment2.sam
bowtie2 --fast -x Tbbgenome -1 ./219_L8_1.fq -2 ./219_L8_2.fq -S alignment3.sam
bowtie2 --fast -x Tbbgenome -1 ./220_L8_1.fq -2 ./220_L8_2.fq -S alignment4.sam
bowtie2 --fast -x Tbbgenome -1 ./221_L8_1.fq -2 ./221_L8_2.fq -S alignment5.sam
bowtie2 --fast -x Tbbgenome -1 ./222_L8_1.fq -2 ./222_L8_2.fq -S alignment6.sam
#convert the output to indexed "bam" format.
samtools view -bS alignment1.sam > alignment1.bam
samtools view -bS alignment2.sam > alignment2.bam 
samtools view -bS alignment3.sam > alignment3.bam
samtools view -bS alignment4.sam > alignment4.bam 
samtools view -bS alignment5.sam > alignment5.bam
samtools view -bS alignment6.sam > alignment6.bam 
samtools sort alignment1.bam -o alignment1.sorted.bam
samtools sort alignment2.bam -o alignment2.sorted.bam
samtools sort alignment3.bam -o alignment3.sorted.bam
samtools sort alignment4.bam -o alignment4.sorted.bam
samtools sort alignment5.bam -o alignment5.sorted.bam
samtools sort alignment6.bam -o alignment6.sorted.bam
...
awk 'NF==1 {next} {T = 0; for(i=2; i <= NF; i++) T += $i; T /= (NF-1); print $1, T}' ./slender_samples.txt > ./slender_mean.txt
awk 'NF==1 {next} {T = 0; for(i=2; i <= NF; i++) T += $i; T /= (NF-1); print $1, T}' ./stumpy_samples.txt > ./stumpy_mean.txt
paste ./slender_mean.txt ./stumpy_mean.txt > mean.txt
head -n6 ./mean.txt
